...
**W15. `rewriteInForce()` is one-directional containment, so a deletion-only rewrite always scores 1.0** *(structural; 0 corpus incidence for a reason worth reading)*
